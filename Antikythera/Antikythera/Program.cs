...
namespace ExampleRobotCode
{
   ...
   public class Program
    {​
       ...
       private static FlagState currentIntakeState = FlagState.INIT;
       private static FlagState lastIntakeState = FlagState.UNKNOWN;​
           ...
           int counter = 0;
            ...
            {​
               ...
               if (gamepad.GetButton(0)) // is button with id 0 pressed
                {
                    currentIntakeState = FlagState.IDLE;
                }
                if (gamepad.GetButton(1))
                {
                    currentIntakeState = FlagState.IN;
                }
                if (gamepad.GetButton(2))
                {
                    currentIntakeState = FlagState.IN_SLOW;
                }
                if (gamepad.GetButton(3))
                {
                    currentIntakeState = FlagState.REVERSE;
                }
​
               ...
               if (currentIntakeState != lastIntakeState) // make code not run every update to have faster run time
                {
                    ...
                    switch (currentIntakeState) // switch on current state for intake
                    {
                        case FlagState.INIT: // is it in INIT?
                            talon.Set(ControlMode.PercentOutput, 0.0); // set talon to 0%
                            currentIntakeState = FlagState.IDLE; // set current state to IDLE
                        ...
                        default:
                            ...
                            currentIntakeState = FlagState.IDLE; // set state to idle to prevent further errors
                            break;
                    }
                }
​
